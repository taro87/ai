...
SELECT EMPNO "사번", ENAME 이름, SAL 급여, JOB
    FROM EMP; -- 별칭에 SPACE가 없는 경우는 따음표 생략가능
...
SELECT EMPNO NO, ENAME, SAL FROM EMP WHERE SAL<>3000;
    ...
    -- EX4=3. 부서번호(DEPTNO)가 10번인 사원의 모든 필드
...
SELECT EMPNO, ENAME, SAL "예전월급", SAL*1.1 "현재월급" FROM EMP;
    ...
    SELECT ENAME, SAL, SAL*12 ANNUALSAL
      FROM EMP
     WHERE SAL*12 > 10000 --WHERE절에서는 별칭 사용 불가
     ORDER BY ANNUALSAL;
...
SELECT EMPNO 사번, ENAME 이름, JOB 업무, SAL 급여
  FROM EMP
 WHERE JOB = 'MANAGER' AND SAL >= 2500;
--13.“ename은 XXX 업무이고 연봉은 XX다” 스타일로 모두 출력(연봉은 SAL*12+COMM)
SELECT ENAME || '은' || JOB || '업무이고 연봉은'|| (SAL*12+NVL(COMM, 0)) || '다' MSG
  FROM EMP;
...
-- (4) 필드명 IS NULL : 필드명이 NULL인지를 검색할 때
    -- ex. COMM(상여)이 없는 사원
...
--12.	입사일이 81년도인 직원의 사번, 사원명, 입사일, 업무, 급여를 출력
  SELECT DEPTNO, ENAME, HIREDATE, JOB, SAL
    FROM EMP
